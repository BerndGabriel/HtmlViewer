...
{----------------TGif.GetStripBitmap}
type
  TLayoutType = packed record
    BFH: TBitmapFileHeader;
    BIH: TBitmapInfoHeader;
  end;
  PLayoutType = ^TLayoutType;
...
{find size needed for strip bitmap}
  ...
  FileSize := Sizeof(TLayoutType) + PixelSize;
  ...
  try
    ...
    Pix := PByte(PtrInt(PL) + Sizeof(TLayoutType)); {where pixels start}
    ...
    begin {set up a mask similarly}
      ...
      MPix := PByte(PtrInt(MPL) + Sizeof(TLayoutType)); {where mask pixels start}
